"""
Generate 04_cvc_clinicdb_kaggle.ipynb — a no-touch Kaggle notebook for
CVC-ClinicDB that incorporates all fixes developed for the Kvasir notebook.
Run this script locally: python generate_cvc_notebook.py
"""
import json, uuid, os
...
# ── Cell 0: Title ──────────────────────────────────────────────────────────────
cells.append(md(
"""# 04 · CVC-ClinicDB: EIoU vs AEIoU Thorough Comparison (Kaggle Edition)

Kaggle-adapted experiment for **CVC-ClinicDB** polyp detection dataset.
Mirrors the methodology of `03_kvasir_kaggle.ipynb` with all Kaggle-specific
path and compatibility fixes applied.

**Before running:**
1. Attach the CVC-ClinicDB dataset:
   *Add Data → Search "cvcclinicdb" (balraj98/cvcclinicdb) → Add*
2. Enable GPU: *Settings → Accelerator → GPU T4 x2 (or P100)*
3. Run All Cells — fully automatic, no further input needed.

**Output:** `/kaggle/working/amorphous-yolo/experiments_cvc/`
Download via the *Output* tab after the run completes (~4–5 hours on T4).
""", "cvc-title"))

# ── Cell 1: GPU check ──────────────────────────────────────────────────────────
cells.append(code(
"""# --- Kaggle: no Drive mount needed
# All outputs go to /kaggle/working/ which Kaggle auto-persists.
...
- Dataset `balraj98/cvcclinicdb` attached as input
...
# wandb 0.24.1: optional experiment tracking
...
DATASET_ROOT  = PROJECT_DIR / "datasets" / "cvc-clinicdb"
EXPERIMENTS   = PROJECT_DIR / "experiments_cvc"
...
DRIVE_AVAILABLE = False  # no Drive on Kaggle — kept for code compatibility
...
DEVICE   = 0
MODEL_PT = "yolo26n.pt"
WANDB_PROJECT = "cvc-clinicdb-aeiou"
...
cells.append(md(
"""## Section 2 · CVC-ClinicDB Dataset

### Why CVC-ClinicDB for AEIoU validation?

CVC-ClinicDB (Bernal et al., 2015) contains **612 colonoscopy frames** with
pixel-level polyp segmentation masks. Like Kvasir-SEG, polyps have irregular,
amorphous boundaries — the exact case AEIoU's rigidity penalty targets.

| Property | Value |
|---|---|
| Total images | 612 |
| Train / Val  | ~490 / ~122 (80/20 random split, seed 42) |
| Classes | 1 (polyp) |
| Image size | variable (~384×288) |
...
cells.append(code(
"""# --- Kaggle: check output directory and list any previously completed runs
from pathlib import Path
import pandas as pd
...
# ── Cell 9: Dataset setup from Kaggle input ────────────────────────────────────
cells.append(code(
"""# --- Setup CVC-ClinicDB from Kaggle input dataset
# Add dataset 'balraj98/cvcclinicdb' via:
#   Notebook settings -> Add data -> Search "cvcclinicdb" -> Add
import shutil
from pathlib import Path

CVC_INPUT = Path("/kaggle/input/cvcclinicdb")
...
# ── Auto-detect original images directory ────────────────────────────────────
orig_dir = None
for candidate in [
    CVC_INPUT / "CVC-ClinicDB" / "Original",
    CVC_INPUT / "PNG" / "Original",
    CVC_INPUT / "Original",
    CVC_INPUT,
]:
    if candidate.is_dir():
        imgs = _find_images(candidate)
        if len(imgs) >= 600:
            orig_dir = candidate
            print(f"Found images in: {candidate} ({len(imgs)} files)")
            break

assert orig_dir is not None, (
    f"Could not find CVC images in {CVC_INPUT}. "
    "Please attach 'balraj98/cvcclinicdb' as a Kaggle dataset."
)

# ── Auto-detect masks directory ───────────────────────────────────────────────
mask_dir = None
for cand_root in [CVC_INPUT / "CVC-ClinicDB", CVC_INPUT / "PNG", CVC_INPUT]:
    for mask_name in ["Ground Truth", "masks", "Masks", "GroundTruth", "ground_truth"]:
        candidate = cand_root / mask_name
        if candidate.is_dir():
            msks = _find_images(candidate)
            if len(msks) >= 600:
                mask_dir = candidate
                print(f"Found masks  in: {candidate} ({len(msks)} files)")
                break
    if mask_dir:
        break

assert mask_dir is not None, f"Could not find CVC masks in {CVC_INPUT}."
...
assert n_imgs == n_masks >= 600, f"Expected >=600 matched pairs, got {n_imgs}/{n_masks}"
print("Dataset setup complete.")
""", "cvc-dataset-setup"))

# ── Cell 10: Convert masks -> YOLO labels + train/val split ───────────────────
cells.append(code(
"""# --- Convert CVC-ClinicDB masks -> YOLO bbox labels + 80/20 train/val split
# CVC-ClinicDB provides binary PNG masks (white=polyp, black=background).
# We derive the tight bounding box from the largest connected component.
...
if n_train_check >= 480:
    ...
    for split_name, img_list in splits_map.items():
        ...
        for img_path in img_list:
            stem = img_path.stem
            # Match mask by stem (PNG preferred, then JPG)
            mask_path = None
...
# sigma_low  = 0.02 : mild annotation noise (~2% image dimension)
# sigma_high = 0.08 : severe noise (~8% image dimension)
...
cells.append(code(
"""# --- Create YOLO dataset YAML files at runtime (CVC-specific, no patching needed)
# YAMLs are created fresh pointing to DATASET_ROOT — no hardcoded Colab paths.
from pathlib import Path
...
yaml_defs = {
    "cvc_clinicdb.yaml":      "valid/images",
    "cvc_clinicdb_low.yaml":  "valid_low/images",
    "cvc_clinicdb_high.yaml": "valid_high/images",
}
...
SPLIT_CONFIGS = {
    "clean": DATA_DIR / "cvc_clinicdb.yaml",
    "low":   DATA_DIR / "cvc_clinicdb_low.yaml",
    "high":  DATA_DIR / "cvc_clinicdb_high.yaml",
}
print(f"\\nSPLIT_CONFIGS:")
...
for split_name, cfg_path in SPLIT_CONFIGS.items():
    ...
    status  = "OK" if n_imgs >= 100 else "CHECK"
...
assert n_train >= 480, f"Expected >=480 train images, got {n_train}"
...
loss functions. The original CIoU forward is saved once and always restored after
each training run via `restore_loss()`.

This is identical to the Kvasir notebook — the patch is loss-agnostic.
""", "cvc-sec3-md"))
...
def sync_to_drive(run_name):
    pass  # No Drive on Kaggle — outputs auto-persisted in /kaggle/working/
...
def run_training(loss_name, loss_fn, split_name, yaml_path,
    ...
    # CVC run name prefix
    run_name = f"cvc_yolo26n_{loss_name}_{split_name}_s{seed}_e{epochs}"
...
Estimated time: ~1.75 hours on T4 GPU.
""", "cvc-sec8-md"))
...
ax.set_title("CVC-ClinicDB — Final mAP50-95: All Losses x 3 Splits", fontsize=13, fontweight="bold")
...
fig.suptitle("CVC-ClinicDB — AEIoU Rigidity Sweep vs Baselines", fontsize=13, fontweight="bold")
...
for ax, metric_col, ylabel in [
    ...
    # Baseline reference lines (clean split only for readability)
...
fig.suptitle("CVC-ClinicDB — Learning Curves (clean split)", fontsize=13, fontweight="bold")
...
ax.set_title("CVC-ClinicDB — Noise Robustness Gap (All Losses)", fontsize=13, fontweight="bold")
...
print("=== CVC-ClinicDB Experiment Artifact Summary ===\\n")
...
# ── Cell 33: Section 14 — Metrics Extraction header ───────────────────────────
cells.append(md(
...
Uses `DetectionValidator` directly (not `model.val()`) with `get_cfg` to ensure
`save_json=True` takes effect and `jdict` is populated for pycocotools.

Image IDs are derived with `hashlib.md5` consistently in both Cell A (GT JSON)
and Cell B (predictions lookup) to avoid pycocotools ID mismatches.
""", "cvc-sec14-md"))
...
def _img_id(stem: str) -> int:
    \"\"\"Stable integer image ID from filename stem.
    Uses int() for purely numeric stems (CVC: '1','2',...,'612'),
    otherwise hashlib.md5 for string stems. MUST match Cell B.
    \"\"\"
    if stem.isdigit():
        return int(stem)
...
# Uses get_cfg(DEFAULT_CFG, overrides) so save_json=True is honoured and
# jdict is populated for pycocotools. Image IDs use _img_id() defined in Cell A.
...
def compute_and_persist_metrics(run_name, weights_path, yaml_path,
    ...
    if pred_json and pred_json.exists():
        try:
            # Remap image_id strings -> ints using _img_id (MUST match Cell A)
    ...
    conf_out = {}
    try:
        cm     = validator.confusion_matrix   # NOT validator.metrics.confusion_matrix
...
for _loss_name in _all_loss_keys:
    for _split_name, _yaml_path in SPLIT_CONFIGS.items():
        _run_name = f"cvc_yolo26n_{_loss_name}_{_split_name}_s{_seed}_e{EPOCHS}"
...
print("=== COCO AP Suite — CVC-ClinicDB clean split ===")
...
# ── Cell 39: Cross-dataset EIoU vs AEIoU summary ──────────────────────────────
cells.append(code(
"""# --- CVC EIoU vs AEIoU summary (for cross-dataset analysis)
import pandas as pd

eiou_rows = []
for _loss_name in _all_loss_keys:
    for _split_name in SPLIT_CONFIGS:
        _run_name = f"cvc_yolo26n_{_loss_name}_{_split_name}_s{SEEDS[0]}_e{EPOCHS}"
        m = all_metrics.get(_run_name, {})
        if m.get("map50_95") is None:
            continue
        eiou_rows.append({
            "dataset": "cvc",
            "dataset_label": "CVC-ClinicDB",
...
df_cvc = pd.DataFrame(eiou_rows)

# Find EIoU baseline and best AEIoU per split
summary_rows = []
for split in ["clean", "low", "high"]:
    sub = df_cvc[df_cvc["split"] == split]
    ...
    summary_rows.append({
        "dataset": "cvc", "dataset_label": "CVC-ClinicDB",
...
out_csv = EXPERIMENTS / "cvc_eiou_vs_aeiou_summary.csv"
df_summary.to_csv(out_csv, index=False)
print("=== CVC-ClinicDB: EIoU vs Best AEIoU ===")
print(df_summary.to_string(index=False))
print(f"\\nSaved -> {out_csv}")
""", "cvc-eiou-vs-aeiou"))

# ── Cell 40: Final Kaggle output ───────────────────────────────────────────────
cells.append(code(
"""# --- Final Kaggle output summary
print("=== CVC-ClinicDB Run Complete ===")
print(f"Experiment dir : {EXPERIMENTS}")
...
print("  Kaggle -> Output tab -> amorphous-yolo/experiments_cvc/")
print("  Key files:")
...
print("    cvc_eiou_vs_aeiou_summary.csv — EIoU vs AEIoU cross-split summary")
""", "cvc-final-summary"))
